...
foreach ($_POST['registered'] as $registered)
    ...
    $update_query = "update course_registration set status = 'Accepted' where student_id = '$registered'";
...
foreach ($_POST['rejected'] as $rejected)
    {
    foreach ($_SESSION['course'] as $course){
    $update_query1 = "update course_registration set status = 'Rejected' where student_id = '$rejected'";
